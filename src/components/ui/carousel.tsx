...
function CarouselDots({ className, ...props }: React.ComponentProps<"div">) {
  ...
  return (
    <div
      role="tablist"
      className={cn(
        "absolute bottom-0 w-full flex items-center justify-center gap-1 lg:gap-2",
        className
      )}
      {...props}
    >
      {api?.scrollSnapList().map((_, index) => (
        <button
          ...
          className={cn(
            "size-1.5 lg:size-2.5 rounded-full z-10 cursor-pointer -translate-y-2",
            index === selectedIndex ? "bg-[#FFFFFF]" : "bg-[#D9D9D9]"
          )}
